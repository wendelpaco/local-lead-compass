...
export function TemperatureBadge({ temperature, size = "sm" }: { temperature: LeadTemperature; size?: "xs" | "sm" }) {
  const { cls, icon: Icon } = map[temperature];
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-md font-medium", cls, size === "xs" ? "px-1.5 py-0.5 text-[10px]" : "px-2 py-0.5 text-xs")}>
...
export function ScoreBadge({ score }: { score: number }) {
  const color = score >= 75 ? "text-hot" : score >= 45 ? "text-warm-foreground" : "text-muted-foreground";
  return (

...
const CITIES: Record<string, { state: string; lat: number; lng: number; neighborhoods: string[] }> = {
  "Porto Alegre": { state: "RS", lat: -30.0346, lng: -51.2177, neighborhoods: ["Centro Histórico", "Moinhos de Vento", "Cidade Baixa", "Menino Deus", "Petrópolis", "Bom Fim", "Auxiliadora", "Higienópolis"] },
  "São Paulo": { state: "SP", lat: -23.5505, lng: -46.6333, neighborhoods: ["Pinheiros", "Vila Madalena", "Moema", "Itaim Bibi", "Jardins", "Vila Mariana", "Tatuapé", "Perdizes"] },
  "Rio de Janeiro": { state: "RJ", lat: -22.9068, lng: -43.1729, neighborhoods: ["Copacabana", "Ipanema", "Leblon", "Botafogo", "Tijuca", "Barra da Tijuca", "Flamengo", "Laranjeiras"] },
  "Curitiba": { state: "PR", lat: -25.4284, lng: -49.2733, neighborhoods: ["Batel", "Água Verde", "Centro", "Cabral", "Bigorrilho", "Rebouças"] },
  "Belo Horizonte": { state: "MG", lat: -19.9167, lng: -43.9345, neighborhoods: ["Savassi", "Funcionários", "Lourdes", "Buritis", "Pampulha"] },
  "Florianópolis": { state: "SC", lat: -27.5954, lng: -48.548, neighborhoods: ["Centro", "Trindade", "Lagoa da Conceição", "Ingleses", "Coqueiros"] },
};

const NAME_TEMPLATES: Record<string, string[]> = {
  "Clínica médica": ["Clínica Nova Vida", "Clínica Horizonte", "Saúde Mais", "Instituto MedCare", "Vitalis Medicina"],
  "Clínica odontológica": ["Odonto Prime", "Instituto Sorriso", "Sorrir Odontologia", "OrthoCenter", "DentClin"],
  "Salão de beleza": ["Studio Bella", "Espaço Beauté", "Charme Salão", "Bellíssima Studio", "Lumière Beleza"],
  "Barbearia": ["Barbearia Central", "The Barber Co.", "Barbearia do Beto", "Old School Barber", "Barba & Cia"],
  "Academia": ["Academia Movimento", "Fit Club", "Corpo Forte", "Body Center", "Energy Fitness"],
  "Restaurante": ["Restaurante Villa Verde", "Sabor da Casa", "Cantina Bella Napoli", "Grelha Sul", "Cozinha Mineira"],
  "Pet shop": ["PetCare Veterinária", "Pet Amigo", "Mundo Pet", "PatinhaFeliz", "Cãopanhia Pet"],
  "Escritório de advocacia": ["Silva & Associados", "Advocacia Ferreira", "Costa Advogados", "Jurídico Brasil"],
...
function makeLead(companyName: string, category: string, cityName: string, seed: number): Lead {
  ...
  const distanceKm = Number((Math.sqrt(Math.pow((lat - city.lat) * 111, 2) + Math.pow((lng - city.lng) * 100, 2))).toFixed(1));
  ...
  const stateName = { RS: "Rio Grande do Sul", SP: "São Paulo", RJ: "Rio de Janeiro", PR: "Paraná", MG: "Minas Gerais", SC: "Santa Catarina" }[city.state] ?? city.state;
  const domainSlug = companyName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "");
  ...
  return {
    ...
    stage,
    estimatedValue,
    lastInteractionAt: rand() > 0.6 ? new Date(Date.now() - Math.floor(rand() * 10) * 86400000).toISOString() : undefined,
    discoveredAt,
...
export function generateLeads(): Lead[] {
  ...
  // extra to reach 50+
  const extra = ["Estúdio Ateliê", "Centro Médico Aurora", "Espaço Zen", "Casa & Estilo", "Motor Prime", "Bella Vida Salão", "Estética Alma", "Odonto Feliz", "Med Center", "Corpo em Foco"];
